...
public class RNMailModule extends ReactContextBaseJavaModule {
    ...
    public void mail(ReadableMap options, Callback callback) {
        ...
        if (options.hasKey("attachment") && !options.isNull("attachment")) {
            ...
            for (int j = 0; j < attachments.size(); j++) {
                ...
                if (attachment.hasKey("path") && !attachment.isNull("path")) {
                    ...
                    String authority = reactContext.getApplicationContext().getPackageName() + ".provider";
                    Uri p = FileProvider.getUriForFile(reactContext, authority, file);
                    uris.add(p);
                }
            }
        ...
        if (list.size() == 1) {
            ...
            chooser.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_GRANT_READ_URI_PERMISSION);
